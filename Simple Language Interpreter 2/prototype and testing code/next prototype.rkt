...
(define M_while
  (lambda (stmt state return next prev_break continue)
       (loop (condition stmt) (loop_body stmt) state return next
             (lambda (break_state) (next break_state)) continue) continue))                                    ; <-- This is where we specify our break loop continuation.
...
(define M_block
  (lambda (stmts state return next break continue)
    (let ([new_next (lambda (next_state) (M_state (next_stmt stmts) next_state return next break continue))])
      (cond
        [(eq? (curr_inner_stmt stmts) 'while) (M_while (curr_stmt stmts) state return new_next break continue)]
        [(eq? (curr_inner_stmt stmts) 'if) (M_if (curr_stmt stmts) state return new_next break continue)]
